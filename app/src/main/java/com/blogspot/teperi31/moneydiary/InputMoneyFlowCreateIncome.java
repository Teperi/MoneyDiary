...
public class InputMoneyFlowCreateIncome extends AppCompatActivity {
	...
	protected void onCreate(@Nullable Bundle savedInstanceState) {
		...
		ArrayAdapter categoryAdapter = new ArrayAdapter(this, android.R.layout.simple_spinner_item, ApplicationClass.mfIncomeCategoryList);
		categoryAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
		spinnerCategory.setAdapter(categoryAdapter);
		
		
		// 수입 지출 이체 버튼 변경
		Button mButtonClicked= findViewById(R.id.input_moneyflow_type_income);
		...
		findViewById(R.id.input_moneyflow_type_expense).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Intent i = new Intent(InputMoneyFlowCreateIncome.this, InputMoneyFlowCreateExpense.class);
		...
		findViewById(R.id.input_moneyflow_save).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(inputPrice.getText().toString().trim().length() <= 0) {
					Toast.makeText(InputMoneyFlowCreateIncome.this, "금액을 입력하세요", Toast.LENGTH_SHORT).show();
					
				} else if (inputContent.getText().toString().length() <= 0) {
					ApplicationClass.mfList.add(
							new DataMoneyFlow("수입",
									setDate,
					...
					UtilPreference.setMoneyflow(InputMoneyFlowCreateIncome.this);
					Intent i = new Intent(InputMoneyFlowCreateIncome.this, RecyclerviewMoneyFlow.class);
